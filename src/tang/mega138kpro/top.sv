...
always @(negedge clk_28m) begin
    if( boot_button_detected || !pll_lock || !rom_done || !reset_n || osd_reset || kbd_reset)
        reset_cnt <= 16'hffff;
    else if(reset_cnt != 0)
...
wire spi_io_clk = spi_ext?pmod_companion_clk_F:spi_sclk;
...
always @(posedge clk_28m or negedge pll_lock) begin
   if(!pll_lock || boot_button_detected ) begin
      start_rom_copy <= 1'b0;
      mem_ready_D <= 1'b0;
...
always @(posedge clk_85m or negedge mem_ready) begin
    if(!mem_ready || boot_button_detected ) begin
